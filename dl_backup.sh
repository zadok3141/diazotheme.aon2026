...
cleanup() {
        ...
        # Clean up latest directory if extraction was interrupted
        if [[ "$EXTRACTION_IN_PROGRESS" == true && -d latest ]]; then
...
# First, check for --help before normal option processing
for arg in "$@"; do
        if [ "$arg" = "--help" ]; then
                show_help=true
        fi
...
# Set default values (can be overridden with command-line args)
BUCKET="s3://oag-backups/from_wpintranet03.oag.net/"
MAX_BACKUPS=1
...
display_help() {
        ...
        echo "  -m, --max-backups N  Maximum number of current-backup-* directories to keep (default: $MAX_BACKUPS)"
        ...
        echo "  -f, --force-create-dirs  Create instance/var/filestorage and instance/var/blobs directories if missing"
...
if [[ ! -d instance/var/filestorage || ! -d instance/var/blobs ]]; then
        if [[ "$FORCE_CREATE_DIRS" == true || "$NON_INTERACTIVE" == true && "$FORCE_CREATE_DIRS" == true ]]; then
                ...
                progress_start "Creating instance/var/filestorage directory"
                mkdir -p instance/var/filestorage
                progress_done
                progress_start "Creating instance/var/blobs directory"
                mkdir -p instance/var/blobs
                progress_done
                success "Plone instance directories created successfully."
        elif [[ "$NON_INTERACTIVE" == true ]]; then
                error "This doesn't appear to be a Plone instance directory."
                echo "Required directories: instance/var/filestorage, instance/var/blobs"
                echo "Use --force-create-dirs flag to create them automatically."
                exit 1
        else
                warn "This doesn't appear to be a Plone instance directory."
                echo "Missing directories: instance/var/filestorage and/or instance/var/blobs"
                echo
...
if [[ "$REINSTALL_LATEST" == true ]]; then
        ...
        [[ -f "${LATEST_DATE}"*.fsz ]] && FSZ_EXISTS=true
        ...
        if [[ "$BLOBSTORAGE_EXISTS" == false && ! -d latest ]]; then
                error "No blobstorage found for latest backup date: $LATEST_DATE"
                echo "Required: ${LATEST_DATE}*.index-blobstorage.tgz or ${LATEST_DATE}*.fsz-blobstorage.tgz (or latest/ directory)"
                exit 1
        fi
...
# Remove existing latest directory (skip in reinstall mode to preserve extracted data)
if [[ -d latest && "$REINSTALL_LATEST" != true ]]; then
        progress_start "Removing existing latest directory"
        rm -rf latest
        progress_done
elif [[ -d latest && "$REINSTALL_LATEST" == true ]]; then
        verbose "Reinstall mode: Keeping existing latest directory."
fi
...
BLOBSTORAGE_TGZ=$(detect_blobstorage_file "$FILEDATE")
BLOBSTORAGE_DETECT_STATUS=$?
...
if [[ ! -d latest && -n "$BLOBSTORAGE_TGZ" ]]; then
...
if [[ "$EXTRACTED" == true && -f "$FILENAME" && -d latest ]]; then
        if [[ "$NON_INTERACTIVE" == true ]]; then
                echo "Non-interactive mode: Installing backup."
                # Handle current data backup
                if [[ "$KEEP_CURRENT_BACKUP" == true && -f instance/var/filestorage/Data.fs ]]; then
                        echo "Creating backup of current Data.fs."
                        CURRENT_BACKUP="current-backup-$(date +%Y%m%d%H%M%S)"
                        mkdir -p "$CURRENT_BACKUP"
                        cp instance/var/filestorage/Data.fs* "$CURRENT_BACKUP/" 2>/dev/null || true
                        echo "Current Data.fs backed up to $CURRENT_BACKUP/"

                        # Check for old backups
                        BACKUPS=($(ls -dt current-backup-* 2>/dev/null))
        ...
        rm -f instance/var/filestorage/Data.fs instance/var/filestorage/Data.fs.index instance/var/filestorage/Data.fs.lock instance/var/filestorage/Data.fs.tmp

        echo "Removing current blobstorage content."
        rm -rf instance/var/blobs/0x00 instance/var/blobs/tmp instance/var/blobs/.layout

        echo "Installing new blobstorage."
        if [[ -d latest/blobstorage ]]; then
                cp -r latest/blobstorage/* instance/var/blobs/
                cp latest/blobstorage/.layout instance/var/blobs/ 2>/dev/null || true
        else
        ...
        if [[ -f "$FILENAME" ]]; then
                cp "$FILENAME" instance/var/filestorage/Data.fs
                echo "Data.fs installed successfully."

                # Check for index file
                if [[ -f "$FILENAME.index" ]]; then
                        cp "$FILENAME.index" instance/var/filestorage/Data.fs.index
        ...
        if [[ ! -d latest ]]; then echo "Missing: latest directory"; fi
        exit 1
fi
